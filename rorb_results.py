...
def main():
    ...
    means = df.groupby(['AEP', 'Duration'])[peak].mean()
    means = means.reset_index()
    df_plot = df.copy()
    df_plot = df_plot.drop(axis=1, columns=['Run', 'TPat', 'Rain(mm)', 'ARF'])
    df_plot = df_plot.reset_index()
    fig, axes = plt.subplots(2, 2)
    sns.boxplot(x='Duration', y = peak, hue = 'AEP', data = df_plot, ax = axes[0, 0])
    LP = sns.lineplot(x = 'Duration', y = peak, hue = 'AEP', data = means, ax = axes[0, 1])

    # label points on the plot
    for x, y in zip(means['Duration'], means[peak]):
